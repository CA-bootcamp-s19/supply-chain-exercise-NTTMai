...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require (msg.sender == _address, "The caller is not verify !");
     _;
  }


  modifier paidEnough(uint _price) {
    require(msg.value >= _price, "The paid is not enough !");
    _;
  }
  ...
  modifier forSale(uint _sku) {
    require(items[_sku].price != 0 && items[_sku].state == State.ForSale, "Item is not for sale !");
    _;
  }
  ...
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, "Item is not reveived !");
    _;
  }
